...
def menu_generalisation():
    ...
    if choix == "Optimiser pour approvisionner 100% des villes":
        ...
        if st.button("🔧 Lancer l'optimisation globale"):
            ...
            # Affiche TOUJOURS les travaux réalisés
            st.markdown("**Travaux réalisés :**")
            if travaux:
                for i, ((depart, arrivee), cap, new_flot) in enumerate(travaux, 1):
                    st.write(f"Travaux #{i} : Liaison {depart} ➝ {arrivee} portée à {cap} unités → Flot = {new_flot} unités")
