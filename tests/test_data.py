...
def test_creer_liaison_valide():
    liaison = creer_liaison(
        "A", "B", 50, noms_noeuds={"A", "B", "C"}, liaisons_existantes=[]
    )
    assert isinstance(liaison, Liaison)
...
def test_saisir_liaisons(monkeypatch):
    ...
    inputs = iter(
        [
            ...
            "A",
            "B",
            "-20",  # ❌ capacité invalide
            "A",
            "B",
            "abc",  # ❌ capacité invalide
            "A",
            "B",
            "30",
            "o",  # ✅ Valide
...
def test_creer_liaison_capacite_non_positive():
    with pytest.raises(
        ValueError, match="capacité de la liaison doit être un entier positif"
    ):
...
def test_creer_liaison_doublon():
    ...
    with pytest.raises(ValueError, match="liaison existe déjà"):
        creer_liaison(
            "A", "B", 80, noms_noeuds={"A", "B"}, liaisons_existantes=[liaison1]
        )
